...
public class CriticalPath {
    ...
    //accomodate Heterogenous VMs
    public CriticalPath(TopologicalOrder order, Map<Task, Double> runtimes, Environment environment, VMType vmType) {
        this.eft = new HashMap<Task, Double>();
        
        if (runtimes == null) {
            runtimes = new HashMap<Task, Double>();
            for (Task task : order) {
                runtimes.put(task, environment.getPredictedRuntime(vmType, task));
            }
        }
