...
import cws.core.WorkflowEngineSeparateRPSched;
...
public class DynamicAlgorithm extends Algorithm {
    ...
    private void prepareEnvironment() {
        ...
        setWorkflowEngine(new WorkflowEngineSeparateRPSched (provisioner, scheduler, getBudget(), getDeadline(), getCloudsim()));
    ...
    private void launchVMs(Cloud cloud, WorkflowEngine workflowEngine, int numEstimatedVMs) {
        ...
        for (int i = 0; i < numEstimatedVMs; i++) {
            ...
            vms.add(vm);
            getCloudsim().send(workflowEngine.getId(), cloud.getId(), 0.0, WorkflowEvent.VM_LAUNCH, vm);
        }
    }
