...
public class Slowest extends CombinedDynamicAlgorithm {
	...
	public Slowest(double budget, double deadline, List<DAG> dags, AlgorithmStatistics ensembleStatistics, CloudSimWrapper cloudsim, boolean saveRuntimeData) {
	...
	public void scheduleQueueJobs() {
		...
		for (Job job : jobs){
			if(!jobsProvisioned.containsKey(job)){
				vmType = environment.getCheapestVM();
